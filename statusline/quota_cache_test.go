...
func TestLoadStopsAfterRejectedKeyUntilKeyChanges(t *testing.T) {
	...
	if auths := source.load(observed); auths != nil || proxy.hits != 1 {
		t.Fatalf("first load: %v after %d hits", auths, proxy.hits)
	}
	if source.load(observed.Add(time.Hour)); proxy.hits != 1 {
		t.Errorf("hits = %d, want no retry with the same rejected key", proxy.hits)
	}
	...
	if auths := source.load(observed.Add(time.Hour + time.Second)); len(auths) != 2 || proxy.hits != 2 {
		t.Errorf("load with a new key: %d auths after %d hits, want an immediate refetch", len(auths), proxy.hits)
	}
}
