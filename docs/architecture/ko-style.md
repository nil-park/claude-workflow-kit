...
- `as`에는 직역의 원어, 정서법의 이름, 문체가 들어간다.
  - `컴퓨터 용어에서 consumer의 직역`, `이중 피동`, `과장된 문어체`.
- 대체 표현이 문맥마다 갈리는 항목은 `use`를 비워 둔다.
- `term`은 정규식이다. 특수문자가 없으면 부분 문자열과 같게 동작한다.
  - `소비자`가 `소비자가`를, `재수출`이 `재수출한다`를 잡는다.
- 다른 단어에 파묻히는 표현은 앞뒤로 경계를 건다. 뒤는 붙을 수 있는 조사를 열거한다.
  - `(?<![가-힣])축[은이을의에으과만]`은 `축은`과 `축이라는`을 잡고 `건축이`·`축소`는 거른다.
  - 조사를 안 적고 한 글자를 통째로 받으면 `축소`·`축적`까지 끌려온다.
