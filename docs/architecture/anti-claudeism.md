...
anti-claudeism은 한국어 텍스트의 문체를 검사하는 Hook 스크립트 하나와 Skill 하나로 이루어진다.

| 구성요소            | 종류                   | 실행 시점                                                     | 검사 방식                          |
| ------------------- | ---------------------- | ------------------------------------------------------------- | ---------------------------------- |
| `anti_claudeism.py` | Stop Hook, 명령줄 도구 | 파일을 고친 턴이 끝날 때(Hook), 인자를 주고 실행할 때(명령줄) | 사전에 등록된 정규식과 매칭        |
| `anti-claudeism`    | Skill                  | 호출할 때                                                     | Skill 본문에 적힌 판정 기준과 대조 |

낱말과 표기는 `anti_claudeism.py`가, 문형과 문장 구조는 Skill이 검사한다.
...
- Hook의 종료 코드는 탐지 여부와 무관하게 항상 0이다.
...
- 탐지 결과 한 줄의 형식은 `<path>:<line>  "<term>"이/가 <as>(으)로 쓰였다면 "<use>"로 수정한다.`이다.
  - `<path>`는 파일이 프로젝트 루트 안에 있으면 상대경로로, 밖에 있으면 절대경로로 쓴다.
...
- `anti_claudeism.py`는 Hook으로 실행할 때와 명령줄에서 실행할 때 모두 아래 사전 파일 세 개를 순서대로 읽어 합친다.
...
| 3    | `<프로젝트 루트>/.claude/claudeism-dictionary.json`         | 이 프로젝트           |

- 2번이나 3번 파일이 없으면 오류 없이 건너뛴다.
- 프로젝트 루트는 `CLAUDE_PROJECT_DIR`이 가리키는 경로다.
  - 명령줄에서 실행할 때 `CLAUDE_PROJECT_DIR`이 없으면 현재 작업 디렉터리를 프로젝트 루트로 삼는다.

## 명령줄 실행

```bash
python3 anti_claudeism.py -f docs/queue.md README.md
python3 anti_claudeism.py -r docs
```

| 옵션                       | 검사 대상                                                                 |
| -------------------------- | ------------------------------------------------------------------------- |
| `-f`, `--file <경로>`      | 지정한 파일. 확장자와 파일 이름을 가리지 않는다                           |
| `-r`, `--recursive <경로>` | 디렉터리 아래에서 `.gitignore`에 걸리지 않는 파일. 확장자를 가리지 않는다 |

- 두 옵션은 함께 쓸 수 있다.
- 한 옵션에 경로를 여러 개 지정할 수 있다.
- 같은 파일을 여러 번 지정해도 한 번만 검사한다.
  - `-f`로 지정한 파일이 `-r`로 모은 파일에 다시 들어 있는 경우도 여기에 해당한다.
- `-r`은 `git ls-files --cached --others --exclude-standard`로 파일을 모아 경로 순서대로 검사한다.
  - 추적 중인 파일과, 추적하지 않지만 `.gitignore`에 걸리지 않는 파일이 대상이다.
  - `git ls-files`가 실패하면 `.git` 디렉터리만 빼고 모든 파일을 모은다.
    - 디렉터리가 git 작업 트리 밖에 있거나, `git`이 없거나, git이 저장소 접근을 거부한 경우가 여기에 해당한다.
    - 이때 `.gitignore`를 적용하지 않았다는 경고와 실패 이유를 stderr에 남긴다.
- `-r`은 이름이 `claudeism-dictionary.json`이나 `test_anti_claudeism.py`인 파일을 건너뛴다.
  - 이 두 파일도 `-f`로 지정하면 검사한다.
- 인용 줄과 `⁌⁍` 구간을 제외하는 규칙, 1MB 상한, 인코딩 규칙은 Hook과 같다.
  - `-f`로 지정한 파일이 1MB를 넘거나 텍스트로 읽히지 않으면, 건너뛴 이유를 stderr에 경고로 남긴다.
  - `-r`로 모은 파일은 같은 경우에 경고 없이 건너뛴다.
- 탐지 결과를 stdout에 쓴다.
...
| 0         | 탐지 결과 없음                                                   |
| 1         | 탐지 결과 있음                                                   |
| 2         | 인자 오류, 없는 경로, 탐지 항목이 없는 사전, 실행 중 발생한 예외 |
