...
- 마크다운 파일(`.md`, `.mdx`, `.markdown`)에서는 인용 줄을 검사하지 않는다.
  - 줄 앞의 공백과 목록 표지(`-`, `*`, `+`, `1.`)를 건너뛰고 `>`가 나오면 인용 줄이다.
  - `>`부터 줄 끝까지만 제외한다.
    - `>`가 없는 줄은 인용 바로 아래에 있어도 검사한다.
  - `> [!NOTE]`처럼 첫 줄이 `[!유형]`으로 시작하는 인용 블록(알림 블록)은 검사한다.
    - 인용 블록은 인용이 아닌 줄 다음이나 목록 표지가 붙은 인용 줄에서 새로 시작한다.
...
머리말 한 줄과 빈 줄이 먼저 출력되고, 그 뒤로 탐지 결과가 한 건에 한 줄씩 이어진다. 아래 예시에서는 머리말을 생략했다.
...
- 머리말은 낱말만 바꾸지 말고 문장을 새로 쓰라고 지시한다.
  - 지적이 유효하면 `anti-claudeism` 스킬 본문을 다시 읽고, 이번 턴에 고친 파일을 모두 퇴고하라고 덧붙인다.
