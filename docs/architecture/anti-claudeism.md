...
- `anti_claudeism.py`는 Hook과 명령줄 모두에서 아래 사전 파일 세 개를 순서대로 읽어 합친다.
...
| `-r`, `--recursive <경로>` | 디렉터리 아래의 마크다운 파일(`.md`, `.mdx`, `.markdown`) |

- 두 옵션은 함께 쓸 수 있고, 한 옵션에 경로를 여러 개 줄 수 있다.
  - 같은 파일이 여러 번 지정되면 한 번만 검사한다.
- `-r`은 이름이 `.`으로 시작하는 디렉터리에 들어가지 않는다.
  - 각 디렉터리에서 파일을 이름 순서로 먼저 검사하고, 그다음에 하위 디렉터리를 이름 순서로 검사한다.
...
- 출력은 머리말 없이 탐지 결과만 한 줄에 한 건씩 stdout에 쓴다.
