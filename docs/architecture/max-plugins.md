...
파일을 만들거나 고치는 작업을 작성-리뷰 사이클로 완주시키는 계열. 쓰는 단계마다 걸리는
리뷰는 전문 서브 에이전트에 병렬로 흩뿌리고 메인이 검증해 확정한다. 커버리지를 위해
토큰을 아끼지 않는다.
...
파일을 만들거나 고치는 모든 작업이 대상이다. "CLAUDE.md 개정"처럼 가벼운 단위도 사이클을
완주한 뒤 완성본으로 올라간다. 읽기만 하는 조사·분석은 대상이 아니다.

리포지토리 안이면 워킹 트리에서 돌고 커밋 지점은 사용자나 전달 스킬이 정한다. **밖이면
되돌릴 git이 없으므로** 세션 스크래치패드의 사본에서 작업해 원본을 사이클 내내 그대로
두고, 제시 단계에서 승인을 받은 뒤 백업본을 뜨고 교체한다. `.refs/`도 리포지토리 안의
규약이므로 그때는 라운드 로그도 스크래치패드로 간다.

| 단계        | 하는 일                                          | 사용자와 |
| ----------- | ------------------------------------------------ | -------- |
| 1 작업 단위 | 무엇을 바꾸는 작업이고 어떤 도메인인지 합의      | 대화     |
| 2 설계      | 규모에 따라 스펙 문서 또는 한 문단짜리 의도 진술 | 대화     |
| 3 구현      | 메인이 직접 쓴다. 에이전트에 나누는 것은 리뷰뿐  | 없음     |
| 4 제시      | 완주한 결과물을 올린다                           | 대화     |

**리뷰는 단계가 아니라 게이트다.** 무언가를 써서 내놓는 단계는 그것을 대상으로
`review-criteria`를 불러 빈 라운드 3연속까지 돈 뒤에야 다음으로 넘어간다 — 설계는 스펙
문서를, 구현은 구현 변경분을 대상으로. 별도 단계로 두지 않은 것은 예외를 만들지 않기
위해서다. 새로 쓰는 것이 없는 1·4단계에는 걸리지 않는다.

1단계의 도메인 판정이 두 가지를 결정한다 — 조건부 리뷰어를 투입할지, 그리고 리뷰어 표
밖에 남아 메인이 직접 볼 축이 무엇인지. 판정은 메인이 하고 결과를 1단계에서 알린다.

**라운드는 매번 보고하지 않는다.** 메인이 지적을 받고, 고치고, 다시 돌린다. 사용자를
...
`work-cycle`의 라운드가 도는 것이 이것이다 — 설계 라운드의 스펙 문서도 아직 커밋 전이라
같은 자리에 있다. **수집은 메인이 한 번만 한다.** 절차는 `review-criteria` 스킬에 있다.
