...
1단계의 도메인 판정이 두 가지를 결정한다 — 4단계에 조건부 리뷰어를 투입할지, 그리고
리뷰어 표 밖에 남아 메인이 직접 볼 축이 무엇인지. 판정은 메인이 하고 결과를 1단계에서
알린다.
...
리뷰어 표는 고정이라 그 사이로 빠지는 축이 남는다. 그것이 **메인 자신의 리뷰 축**이고,
팬아웃 결과를 기다리는 동안 본다. 메인이 찾은 것도 에이전트가 올린 것과 똑같이 검증을
거쳐야 대장에 오른다. 어떤 축이 거기 해당하는지는 `review-criteria` 스킬에 있다.
