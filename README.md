...
부르므로 그것에 의존한다(설치 시 자동으로 함께 설치·활성화됨).
...
스킬 사이의 의존은
[docs/architecture/skill-dependency.md](docs/architecture/skill-dependency.md)에 있다.
...
| `--squash-before-merge --remove-source-branch` | git-workflow             | 머지 전략·소스브랜치 유지 정책을 강제               | 없음                          |
...
| `docs/` 7분류와 what/how/why 배분               | docs-standards                     | 다른 문서 레이아웃과 충돌 (단, "repo 기존 관례 우선"으로 완화)      |
...
한 플러그인이 다루므로 환경에 따라 다른 것을 고를 필요는 없다.
