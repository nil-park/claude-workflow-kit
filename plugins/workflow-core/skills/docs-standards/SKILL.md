...
| `docs/reference/`    | 외부 플랫폼·라이브러리가 정한 계약. 우리가 바꿀 수 없는 사실              | what                   | why, 우리 판단 |
...
- **`docs/reference/`와 나머지 `docs/` 분류를 가르는 것은 내용을 정하는 주체다.** 우리 구현이 바뀔 때 같이 바뀌는 것이면 나머지 중 하나이고, 플랫폼이 바꿀 때만 바뀌는 것이면 reference다.
  - 출처 URL과 확인 시점을 적는다. 플랫폼이 예고 없이 바꾸므로 언제 기준인지가 내용의 일부다.
  - 공식 문서로 확인되지 않아 실측한 것은 실측이라고 밝힌다.
  - 그 계약을 딛고 우리가 둔 가정은 development에 쓰고 reference에서는 링크만 건다.
