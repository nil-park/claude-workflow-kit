---
name: bootstrap-duo-session
description: >-
  duo-session 스킬을 현재 프로젝트에 설치하거나 업데이트하고 싶을 때 부른다. Claude Code 전용이다.
---

이 스킬이 로드될 때 시스템이 알려주는 베이스 디렉터리(`Base directory for this skill`) 아래
`duo-session.md`를 읽어 프로젝트에 설치하거나 업데이트한다.

- Codex에서 불렀다면 설치하지 않고, 이 스킬이 Claude Code 전용이라고 알린다.
- 설치본은 `.agents/skills/`에 쓰지 않고, 심볼릭 링크 없이 `.claude/skills/duo-session/SKILL.md`에 쓴다.
  - Codex는 `.agents/skills/`에서 스킬을 읽는다.

## 사전 설치 확인

`duo-session`은 본문에서 아래 스킬을 이름으로 참조한다. 설치 또는 업데이트를 시작하기 전에 각
...
`.claude/skills/duo-session/SKILL.md`가 없을 때 실행한다.

1. 베이스 디렉터리 아래 `duo-session.md`를 읽는다.
2. `.claude/skills/duo-session/SKILL.md`에 내용을 쓴다.
3. 완료 후 설치 결과를 보고한다.

## 업데이트

`.claude/skills/duo-session/SKILL.md`가 이미 있을 때 실행한다.

1. 베이스 디렉터리 아래 `duo-session.md`(템플릿)를 읽는다.
2. `.claude/skills/duo-session/SKILL.md`(현재 파일)를 읽는다.
